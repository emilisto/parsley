...
var Chord = function(commands) {
  ...
  var resulting = this.resulting = new Command(identity);
  commandset.link(resulting);
  commandset.save();
...
_.extend(Chord.prototype, {
  ...
  _dispatchChord: function _dispatchChord(result) {
    ...
    _.each(this.commands, function(id) {
      self.commandqueue.enqueueId(id);
    });
  }
});
...
var ChordMap = function(command) {
  Command.call(this, this.dispatchChordmap);
...
_.extend(ChordMap.prototype, {
  ...
  chordmapTask: function chordmapTask(workset) {
    var Command = this.Parsley.Command,
        Chord = this.Parsley.Canvas.Chord;

    console.log(workset);
    //var commands = _.map(workset
    //var chord = new this.Parsley.Canvas.Chord();
  }
});
