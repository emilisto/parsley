...
CommandSet.fetch = function(id, callback) {
  ...
  logger.log('debug', 'CommandSet.fetch() id=%s', id);
  r.hgetall(key('commandset', id), function(err, data) {
    if(err) return callback(err);
...
CommandSet.finishCommand = function(commandSetId, command) {
  ...
  var r = redis.get();
  incrementAndGetCounts(commandSetId, function(err, n_total, n_completed) {
