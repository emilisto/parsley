...
_.extend(Command.prototype, LinkableMixin, {
  ...
  listen: function() {
    ...
    this.listener.on('message', function(channel, id, ret) {
      if(id === self.id) {
  ...
  run: function() {
    ...
    try {
      // TODO: use a callback for this instead, to facilitate more complex runs
      var ret = this.task.fn.apply(this, this.arguments);
...
_.extend(CommandSet.prototype, LinkableMixin, {
  ...
  _finish: function() {
    ...
    // TODO: Collect arguments from all finished commands, and pass to dispatchCallbacks()
