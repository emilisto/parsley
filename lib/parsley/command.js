...
var Command = function Command(task) {
  ...
  this.log = _.bind(logger.log, logger);
...
_.extend(Command.prototype, LinkableMixin, {
  ...
  clone: function() {
    ...
    var data = s.deserialize(s.serialize(this));
    delete data.id;
    var command = new Command();
    _.extend(command, data);
    return command;
  },
  ...
  run: function() {
    ...
    this.log('info', 'running command %s with arguments: %s', common.shortId(this.id), args);

    this.emit('run:before', this);
    Runner.run(this.task.fn, this, this.arguments, function(err, ret) {

      if(err) {
        this.error = err;
        this.log('error', 'task %s failed with error: %s', common.shortId(this.id), this.error);
        this.status = 'failed';
  ...
  serializationMap: {
    ...
    // FIXME: register these as serializers in the top instead, so they can be reused.
    'commandset': {
...
Command.prototype.save = function(callback) {
  ...
  this.log('debug', 'saved command %s', common.shortId(this.id));
...
Command.prototype.fetch = function(id, callback) {
  ...
  redis.runScript('fetch-command', 1, [ _key ], function(err, data) {
    ...
    self.log('debug', 'fetched task %s(%s)', common.shortId(self.id), self.task.name);
...
// FIXME: This is a terribly inefficient way to do it - we load it, change it,
// save it, and finally load it again when doing the dispatch. A much smarter
// way would be to leverage Redis' data structures and use actual redis lists
// for callbacks and arguments.
Command.dispatchIdWithArgs = function(id, args) {
  var command = new Command();
...
CommandSet.finishCommand = function(commandSetId, command) {
  ...
  logger.log('debug', 'marking command %s as finished in commandset %s',
             common.shortId(command.id), common.shortId(commandSetId));

  var key = CommandSet.key(commandSetId);
  incrementAndGetCounts(key, function(err, n_total, n_completed) {

    if(n_total === 0) {
      logger.log('error', 'commandset %s is empty', common.shortId(commandSetId));
      return;
    }

    logger.log('debug', 'commandset %s has n_total=%d and n_completed=%d',
               common.shortId(commandSetId), n_total, n_completed);

    // CommandSet is not finished yet
    if (n_completed === n_total) {
      logger.log('info', 'commandset %s finished all %d commands', common.shortId(commandSetId), n_total);
    ...
    if(n_completed > n_total) {
      throw new Error('n_completed > n_total for commandset ' + commandSetId);
    }
