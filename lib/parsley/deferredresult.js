...
var ensureListening = function() {
  ...
  listener.on('message', function(_channel, data) {
    ...
    var objects = commandIdInstanceMap[data.id] || [];
    _.each(objects, function(object) {
      object.setResult(data.error, data.result);
    });
  });
...
var DeferredResult = function(commandId) {
  ...
  if(commandId) {
    if(!commandIdInstanceMap[commandId]) commandIdInstanceMap[commandId] = [];
    commandIdInstanceMap[commandId].push(this);
  }
...
_.extend(DeferredResult.prototype, {
  ...
  publish: function(error, result) {
    ...
    logger.log('info', 'publishing result of %s to %s', common.shortId(this.commandId), channel);
